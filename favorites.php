...
function logError($message) {
    file_put_contents('debug.log', date('Y-m-d H:i:s') . " - Favorites Error: $message\n", FILE_APPEND);
}
...
if (isset($_SESSION['favorites']) && !empty($_SESSION['favorites'])) {
    try {
        // Validate and sanitize favorite IDs
        $valid_favorite_ids = array_filter($_SESSION['favorites'], function($id) use ($pdo) {
        ...
        // Update session with valid IDs only
        $_SESSION['favorites'] = array_values($valid_favorite_ids);

        if (!empty($valid_favorite_ids)) {
            $placeholders = implode(',', array_fill(0, count($valid_favorite_ids), '?'));
            $stmt = $pdo->prepare("SELECT id, name, description, price, image FROM products WHERE id IN ($placeholders) AND stock > 0");
            $stmt->execute($valid_favorite_ids);
            $favorite_products = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } else {
            $_SESSION['errors'] = ["Không có sản phẩm yêu thích hợp lệ."];
        }
    } catch (PDOException $e) {
        logError("DB Error: " . $e->getMessage());
        $_SESSION['errors'] = ["Lỗi cơ sở dữ liệu: " . htmlspecialchars($e->getMessage())];
    }
}
...
?>
    ...
    <head>
        ...
        <title>Sản Phẩm Yêu Thích - KidsToyLand</title>
        <!-- Bootstrap CSS -->
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    ...
                    <div class="d-flex align-items-center">
                        <form class="search-form me-3">
                            <input class="form-control" type="search" placeholder="Tìm kiếm đồ chơi..." aria-label="Search">
    ...
    <main>
        ...
        <section class="favorites-section py-5">
            <div class="container">
                <h2 class="text-center mb-5 fw-bold">Sản Phẩm Yêu Thích</h2>
                <?php
                            ...
                            <div class="col">
                                <div class="card product-card h-100">
                                    ...
                                    <div class="card-footer bg-white border-top-0 d-flex justify-content-between align-items-center">
                                        <button class="btn btn-sm btn-outline-dark favorite-btn favorited" data-product-id="<?php echo $product['id']; ?>">
                                            <i class="fas fa-heart"></i>
                                        </button>
                ...
                try {
                    ...
                    if (data.success) {
                        if (data.is_favorited) {
                            ...
                            // Remove product card from DOM if on favorites page
                            if (window.location.pathname.includes('favorites.php')) {
                                button.closest('.col').remove();
                                // Check if no products remain
                                if (!document.querySelector('.favorites-section .col')) {
                                    document.querySelector('.favorites-section .container').innerHTML += '<p class="text-center">Bạn chưa có sản phẩm yêu thích nào.</p>';
                                }
                            }
                        }
