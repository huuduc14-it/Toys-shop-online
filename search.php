...
function logError($message) {
    file_put_contents('debug.log', date('Y-m-d H:i:s') . " - Search Error: $message\n", FILE_APPEND);
}
...
if (isset($_GET['query']) && !empty(trim($_GET['query']))) {
    try {
        $search_query = filter_var(trim($_GET['query']), FILTER_SANITIZE_STRING);
        $search_term = '%' . $search_query . '%';
        $stmt = $pdo->prepare("SELECT id, name, description, price, image FROM products WHERE (name LIKE ? OR description LIKE ?) AND stock > 0");
...
?>
    ...
    <head>
        ...
        <title>Kết Quả Tìm Kiếm - KidsToyLand</title>
        <!-- Bootstrap CSS -->
                ...
                <div class="collapse navbar-collapse" id="navbarNav">
                    ...
                    <div class="d-flex align-items-center">
                        <form class="search-form me-3" action="search.php" method="GET">
                            <input class="form-control" type="search" name="query" placeholder="Tìm kiếm đồ chơi..." aria-label="Search" value="<?php echo htmlspecialchars($search_query); ?>">
    ...
    <main>
        ...
        <section class="search-results-section py-5">
            <div class="container">
                <h2 class="text-center mb-5 fw-bold">Kết Quả Tìm Kiếm: "<?php echo htmlspecialchars($search_query); ?>"</h2>
                <?php
                ...
                <?php elseif (empty($search_results)): ?>
                    <p class="text-center">Không tìm thấy sản phẩm nào phù hợp với từ khóa "<?php echo htmlspecialchars($search_query); ?>".</p>
                <?php else: ?>
