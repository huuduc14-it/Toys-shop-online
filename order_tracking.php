...
function logError($message) {
    file_put_contents('debug.log', date('Y-m-d H:i:s') . " - Order Tracking Error: $message\n", FILE_APPEND);
}

// Fetch user's orders
try {
    $stmt = $pdo->prepare("
        SELECT id, total_amount, status, order_date, recipient_name, phone, address
        FROM orders
        WHERE user_id = ?
...
?>
    ...
    <head>
        ...
        <title>Theo Dõi Đơn Hàng - KidsToyLand</title>
        <!-- Bootstrap CSS -->
    ...
    <main>
        <section class="py-5">
            <div class="container">
                <h2 class="text-center mb-5 fw-bold">Theo Dõi Đơn Hàng</h2>
